...
import net.runelite.client.plugins.microbot.nirimixology.enums.PotionType;
...
@PluginDescriptor(
        name = PluginDescriptor.NIRI + "Mixology",
        description = "Automated mixology minigame script",
        tags = {"herblore", "microbot", "mixology"},
        enabledByDefault = false
)
@Slf4j
public class NiriMixologyPlugin extends Plugin {

    @Inject
    private Client client;
    @Inject
    private NiriMixologyConfig config;
    @Inject
    private OverlayManager overlayManager;
    @Inject
    private ClientThread clientThread;
    @Inject
    private NiriMixologyOverlay overlay;
    @Inject
    ...
    @Inject
    NiriMixologyScript mixologyScript;

    public NiriMixologyPlugin() {
    }
    ...
    @Provides
    NiriMixologyConfig provideConfig(ConfigManager configManager) {
        return (NiriMixologyConfig) configManager.getConfig(NiriMixologyConfig.class);
    }
    ...
    private void updatePotionOrders() {
        ...
        if (config.onlyLyePotions()) {
            mixologyScript.potionOrders.sort(
                Comparator.comparing((PotionOrder order) -> !order.potionType().containsLye())
                    .thenComparingInt(order -> mixologyScript.customOrder.indexOf(order.potionModifier()))
            );
        } else {
            mixologyScript.potionOrders.sort(Comparator.comparingInt(order -> mixologyScript.customOrder.indexOf(order.potionModifier())));
        }

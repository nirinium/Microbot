...
import net.runelite.client.plugins.microbot.nirimixology.enums.AgaHerbs;
import net.runelite.client.plugins.microbot.nirimixology.enums.LyeHerbs;
import net.runelite.client.plugins.microbot.nirimixology.enums.MoxHerbs;

@ConfigGroup("nirimixology")
@ConfigInformation("<p>Start the script at the bankchest of the mixology minigame room with an empty inventory.</p> <br />" +
        ...
        "</ol>")
public interface NiriMixologyConfig extends Config {
    @ConfigSection(
            name = "Refiner",
    ...
    @ConfigItem(
            keyName = "onlyLyePotions",
            name = "Only Lye Potions",
            description = "Only complete and fulfill potions containing Lye. Completes all potions when no Lye orders are available to refresh the order list.",
            position = 5,
            section = minigame
    )
    default boolean onlyLyePotions() {
        return false;
    }
}

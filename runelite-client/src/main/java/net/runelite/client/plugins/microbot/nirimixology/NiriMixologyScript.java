package net.runelite.client.plugins.microbot.nirimixology;
...
import net.runelite.client.plugins.microbot.nirimixology.enums.AlchemyObject;
import net.runelite.client.plugins.microbot.nirimixology.enums.MixologyState;
import net.runelite.client.plugins.microbot.nirimixology.enums.PotionComponent;
import net.runelite.client.plugins.microbot.nirimixology.enums.PotionModifier;
...
import static net.runelite.client.plugins.microbot.nirimixology.enums.AlchemyObject.MIXING_VESSEL;

public class NiriMixologyScript extends Script {
    ...
    public boolean run(NiriMixologyConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (hasAllFulFilledItems(config)) {
                ...
                switch (mixologyState) {
                    ...
                    case MIX_POTION_STAGE_1:
                        ...
                        boolean shouldOnlyDoLye = config.onlyLyePotions() && hasLyePotionsInOrders();

                        for (PotionOrder _potionOrder : potionOrders) {
                            // Skip non-Lye potions if onlyLyePotions is enabled AND Lye potions exist
                            if (shouldOnlyDoLye && !_potionOrder.potionType().containsLye()) {
                                continue;
                            }
                    ...
                    case MIX_POTION_STAGE_2:
                        ...
                        List<PotionOrder> nonFulfilledPotions;
                        boolean shouldFilterLyeOnly = config.onlyLyePotions() && hasLyePotionsInOrders();
                        
                        if (shouldFilterLyeOnly) {
                            nonFulfilledPotions = potionOrders
                                    .stream()
                                    .filter(x -> !x.fulfilled() && x.potionType().containsLye())
                                    .sorted(Comparator.comparing((PotionOrder order) -> !order.potionType().containsLye())
                                            .thenComparingInt(order -> customOrder.indexOf(order.potionModifier())))
                        ...
                        break;
                    case CONVEYER_BELT:
                        // Only check for fulfilled Lye potions if onlyLyePotions is enabled AND Lye potions exist
                        boolean shouldCheckLyeOnly = config.onlyLyePotions() && hasLyePotionsInOrders();
                        boolean hasFulfilledPotions = shouldCheckLyeOnly
                            ? potionOrders.stream().anyMatch(x -> x.potionType().containsLye() && Rs2Inventory.hasItem(x.potionType().getFulfilledItemId()))
                            : potionOrders.stream().anyMatch(x -> Rs2Inventory.hasItem(x.potionType().getFulfilledItemId()));
                        
                        if (!hasFulfilledPotions) {
                            mixologyState = MixologyState.MIX_POTION_STAGE_1;
                            return;
                        }
    ...
    private boolean hasAllFulFilledItems(NiriMixologyConfig config) {
        Map<Integer, Integer> itemsToCheck = new HashMap<>();
        ...
        for (PotionOrder _potionOrder : potionOrders) {
            // Skip non-Lye potions if onlyLyePotions is enabled AND Lye potions exist
            if (shouldCheckLyeOnly && !_potionOrder.potionType().containsLye()) {
                continue;
            }
    ...
    private boolean hasLyePotionsInOrders() {
        return potionOrders.stream().anyMatch(order -> order.potionType().containsLye());
    }
    ...
    private void createPotion(PotionOrder potionOrder, NiriMixologyConfig config) {

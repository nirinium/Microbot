...
import net.runelite.client.plugins.microbot.nirifighter.enums.AttackStyle;
import net.runelite.client.plugins.microbot.nirifighter.enums.AttackStyleMapper;
...
public class AttackNpcScript extends Script {
    ...
    @SneakyThrows
    public void run(NiriFighterConfig config) {
        try {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (config.centerLocation().distanceTo(Rs2Player.getWorldLocation()) < config.attackRadius() &&
                        !config.centerLocation().equals(new WorldPoint(0, 0, 0)) && NiriFighterPlugin.getState() != State.BANKING) {
                    if (ShortestPathPlugin.getPathfinder() != null)
                        Rs2Walker.setTarget(null);
                    NiriFighterPlugin.setState(State.IDLE);
                }
                ...
                filteredAttackableNpcs.set(
                        Rs2Npc.getAttackableNpcs(config.attackReachableNpcs())
                                .filter(npc -> isInstanced || npc.getWorldLocation().distanceTo(config.centerLocation()) <= config.attackRadius())
                                ...
                                .sorted(Comparator.comparingInt((Rs2NpcModel npc) -> npc.getInteracting() == Microbot.getClient().getLocalPlayer() ? 0 : 1)
                                        .thenComparingInt(npc -> isInstanced
                ...
                if (!NiriFighterPlugin.isWaitingForLoot()) {
                ...
                if (config.toggleWaitForLoot() && !NiriFighterPlugin.isWaitingForLoot() && cachedTargetNpcIndex != -1) {
                    ...
                    if (cachedNpcModel != null && (cachedNpcModel.isDead() || (cachedNpcModel.getHealthRatio() == 0 && cachedNpcModel.getHealthScale() > 0))) {
                        NiriFighterPlugin.setWaitingForLoot(true);
                        NiriFighterPlugin.setLastNpcKilledTime(System.currentTimeMillis());
                ...
                if (config.toggleWaitForLoot() && NiriFighterPlugin.isWaitingForLoot()) {
                    long timeSinceKill = System.currentTimeMillis() - NiriFighterPlugin.getLastNpcKilledTime();
                    int timeoutMs = config.lootWaitTimeout() * 1000;
                    if (timeSinceKill >= timeoutMs) {
                        // Timeout reached, resume combat
                        NiriFighterPlugin.clearWaitForLoot("Loot wait timeout reached, resuming combat");
                ...
                if (Rs2AntibanSettings.antibanEnabled && Rs2AntibanSettings.actionCooldownChance > 0) {
                    if (Rs2AntibanSettings.actionCooldownActive) {
                        NiriFighterPlugin.setState(State.COMBAT);
                ...
                if (!attackableNpcs.isEmpty()) {
                    ...
                    if (Rs2Player.getWorldLocation().isInArea(attackableArea)) {
                        ...
                        if (noNpcCount > 60 && config.slayerMode()) {
                            ...
                            NiriFighterPlugin.addBlacklistedSlayerNpcs(Rs2Slayer.slayerTaskMonsterTarget);
                            noNpcCount = 0;
                            SlayerScript.reset();
                        }
                    } else {
                        Rs2Walker.walkTo(config.centerLocation(), 0);
                        NiriFighterPlugin.setState(State.WALKING);
                    }
     ...
     */
    private void handleItemOnNpcToKill(NiriFighterConfig config) {

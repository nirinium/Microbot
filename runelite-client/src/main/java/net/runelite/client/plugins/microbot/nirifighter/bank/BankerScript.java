package net.runelite.client.plugins.microbot.nirifighter.bank;
...
import net.runelite.client.plugins.microbot.nirifighter.shop.ShopItem;
import net.runelite.client.plugins.microbot.nirifighter.shop.ShopScript;
import net.runelite.client.plugins.microbot.nirifighter.shop.ShopType;
    ...
    public boolean run(NiriFighterConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if(!super.run()) return;
                if (config.bank() && needBanking() && !NiriFighterPlugin.needShopping) {
                    if(handleBanking()){
...
//    }
    ...
    public boolean needBanking() {
        if(config.currentInventorySetup() == null){
            if(config.defaultInventorySetup() != null) {
                NiriFighterPlugin.setCurrentSlayerInventorySetup(config.defaultInventorySetup());
            }
        }        
        if(!config.bank()){
            return false;
    ...
    public void withdrawUpkeepItems(NiriFighterConfig config) {
                    ...
                    if (Rs2Bank.hasBankItem(config.slayerTaskWeaknessItem())) {
                        ...
                        if(Rs2Bank.hasBankItem(config.slayerTaskWeaknessItem(), item.isStackable() ? Rs2Slayer.getSlayerTaskSize() : 1,false)) {
                            ...
                            ShopScript.shopItems.add(new ShopItem(config.slayerTaskWeaknessItem(), itemComp.isStackable() ? Rs2Slayer.getSlayerTaskSize() : 1, itemComp.getId() , isGeItem ? ShopType.GRAND_EXCHANGE : ShopType.SLAYER_SHOP));
                             NiriFighterPlugin.needShopping = true;
                        }
                ...
                if (needProtectiveSlayerEquipment()) {
                    if (Rs2Bank.hasBankItem(Rs2Slayer.getSlayerTaskProtectiveEquipment())) {
                        ...
                        InventorySetup inventorySetup1 = MInventorySetupsPlugin.getInventorySetups().stream().filter(Objects::nonNull).filter(x -> x.getName().equalsIgnoreCase("cache")).findFirst().orElse(null);
                         NiriFighterPlugin.setCurrentSlayerInventorySetup(inventorySetup1);
                    } else {
...
//    public boolean depositAllExcept(NiriFighterConfig config) {
...
//    public boolean isUpkeepItemDepleted(NiriFighterConfig config) {
...
//    }
    ...
    public boolean handleBanking() {
         NiriFighterPlugin.setState(State.BANKING);
        //bankingTriggered = true;
